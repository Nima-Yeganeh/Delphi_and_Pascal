...
uses
  Types, Windows, SysUtils, Classes, Generics.Collections, SyncObjs, StrUtils
  // json:
  {$IFDEF SUPEROBJECT}
  ,superobject
  {$ENDIF}
  ...
  // andremussche:
  ,amdws.IdServerBaseHandling, amdws.IdIOHandlerWebsocket
;
...
type
  ...
  TSocketIOMsg = reference to procedure(const ASocket: ISocketIOContext; const aText: string; const aCallback:
    ISocketIOCallback);
  {$IFDEF SUPEROBJECT}

  TSocketIOMsgJSON = reference to procedure(const ASocket: ISocketIOContext; const aJSON: ISuperObject; const aCallback:
    ISocketIOCallback);
  {$ELSE}

  TSocketIOMsgJSON = reference to procedure(const ASocket: ISocketIOContext; const aJSON: string; const aCallback:
    ISocketIOCallback);
  {$ENDIF}

  TSocketIOEvent = reference to procedure(const ASocket: ISocketIOContext; const aArgument: ISuperArray; const aCallback:
    ISocketIOCallback);
  ...
  ISocketIOContext = interface
    ...
    {$ENDIF}
    procedure EmitEvent(const aEventName: string; const aData: string; const aCallback: TSocketIOMsgJSON = nil; const
      aOnError: TSocketIOError = nil); overload;
  ...
  protected
    ...
    FOnConnectionList, FOnDisconnectList: TSocketIONotifyList;
  ...
  protected
    ...
    procedure WriteSocketIOMsg(const ASocket: ISocketIOContext; const aRoom, aData: string; aCallback:
      TSocketIOCallbackRef = nil; const aOnError: TSocketIOError = nil);
    procedure WriteSocketIOJSON(const ASocket: ISocketIOContext; const aRoom, aJSON: string; aCallback:
      TSocketIOCallbackRef = nil; const aOnError: TSocketIOError = nil);
    procedure WriteSocketIOEvent(const ASocket: ISocketIOContext; const aRoom, aEventName, aJSONArray: string; aCallback:
      TSocketIOCallback; const aOnError: TSocketIOError);
...
//    procedure  EmitEventToAll(const aEventName: string; const aData: ISuperObject; const aCallback: TSocketIOMsgJSON = nil);
  ...
  public
    ...
    procedure Emit(const aEventName: string; const aData: string; const aCallback: TSocketIOMsgJSON = nil; const
      aOnError: TSocketIOError = nil); overload;
    {$IFDEF SUPEROBJECT}
    procedure Emit(const aEventName: string; const aData: ISuperObject; const aCallback: TSocketIOMsgJSON = nil; const
      aOnError: TSocketIOError = nil); overload;
...
uses
  // andremussche:
  amdws.IdServerWebsocketContext, amdws.IdHTTPWebsocketClient
;
...
begin
  Lock;
  FreeAndNil(FSocketIOEventCallback);
  FreeAndNil(FSocketIOEventCallbackRef);
  FreeAndNil(FSocketIOErrorRef);
  //FreeAndNil(FSocketIOEventPromises);

  FreeAndNil(FOnEventList);
  FreeAndNil(FOnConnectionList);
  FreeAndNil(FOnDisconnectList);
...
function TIdBaseSocketIOHandling.NewConnection(const AGUID, aPeerIP: string): ISocketIOContext;
var
...
procedure TIdBaseSocketIOHandling.ProcessCloseChannel(const ASocket: ISocketIOContext; const aChannel: string);
begin
  if aChannel <> '' then
    //todo: close channel
...
procedure TIdBaseSocketIOHandling.ProcessEvent(const AContext: ISocketIOContext; const aText: string; aMsgNr: Integer;
...
var
  ...
  begin
    ...
    //
    inc(xs);
    while (xs <= length(aText)) and (AnsiChar(aText[xs]) in [' ', #13, #10, #8, #9]) do
      inc(xs);
...
procedure TIdBaseSocketIOHandling.ProcessSocketIORequest(const ASocket: ISocketIOContext; const strmRequest: TMemoryStream);
...
procedure TIdBaseSocketIOHandling.ProcessSocketIORequest(const AContext: TIdContext; const strmRequest: TMemoryStream);
var
  socket: ISocketIOContext;
begin
...
begin
  if not FConnectionsGUID.TryGetValue(aGUID, socket) or socket.IsDisconnected then
    socket := NewConnection(aGUID, '');

  if not TSocketIOContext(socket).FConnectSend then
    WriteConnect(socket);

  if (aStrmRequest <> nil) and (aStrmRequest.Size > 0) then
  begin
    aStrmRequest.Position := 0;
    SetLength(bytes, aStrmRequest.Size);
    aStrmRequest.Read(bytes[0], aStrmRequest.Size);

    if (Length(bytes) > 3) and (bytes[0] = 239) and (bytes[1] = 191) and (bytes[2] = 189) then
    begin
...
begin
  ...
  if not FConnections.ContainsValue(socket) and not FConnectionsGUID.ContainsValue(socket) then
  begin
    Lock;
    try
      //ASocket._AddRef;
...
function TIdBaseSocketIOHandling.WriteConnect(const ASocket: ISocketIOContext): string;
var
  notify: TSocketIONotify;
begin
  Lock;
  try
    if not FConnections.ContainsValue(ASocket) and not FConnectionsGUID.ContainsValue(ASocket) then
    begin
      //ASocket._AddRef;
...
procedure TIdBaseSocketIOHandling.WriteDisConnect(const ASocket: ISocketIOContext);
var
  notify: TSocketIONotify;
begin
  if ASocket = nil then
...
procedure TIdBaseSocketIOHandling.WritePing(const ASocket: ISocketIOContext);
begin
...
begin
  ...
  if not Assigned(aCallback) then
    sresult := Format('5:%d:%s:{"name":"%s", "args":%s}', [inr, aRoom, aEventName, aJSONArray])
  else
  begin
...
procedure TIdBaseSocketIOHandling.WriteSocketIOEventRef(const ASocket: ISocketIOContext; const aRoom, aEventName,
  aJSONArray: string; aCallback: TSocketIOCallbackRef; const aOnError: TSocketIOError);
var
  sresult: string;
  inr: Integer;
begin
...
procedure TIdBaseSocketIOHandling.WriteSocketIOJSON(const ASocket: ISocketIOContext; const aRoom, aJSON: string;
  aCallback: TSocketIOCallbackRef = nil; const aOnError: TSocketIOError = nil);
var
  sresult: string;
  inr: Integer;
begin
...
procedure TIdBaseSocketIOHandling.WriteSocketIOMsg(const ASocket: ISocketIOContext; const aRoom, aData: string;
  aCallback: TSocketIOCallbackRef = nil; const aOnError: TSocketIOError = nil);
var
  sresult: string;
  inr: Integer;
...
procedure TIdBaseSocketIOHandling.WriteSocketIOResult(const ASocket: ISocketIOContext; aRequestMsgNr: Integer; const
  aRoom, aData: string);
var
  sresult: string;
begin
...
function TIdBaseSocketIOHandling.WriteString(const ASocket: ISocketIOContext; const aText: string): string;
begin
  if ASocket = nil then
    Exit;
...
constructor TSocketIOCallbackObj.Create(aHandling: TIdBaseSocketIOHandling; aSocket: ISocketIOContext; aMsgNr: Integer);
begin
  FHandling := aHandling;
  FSocket := aSocket;
  FMsgNr := aMsgNr;
...
procedure TSocketIOContext.EmitEvent(const aEventName, aData: string; const aCallback: TSocketIOMsgJSON; const aOnError:
  TSocketIOError);
begin
...
{$IFDEF SUPEROBJECT}
procedure TSocketIOContext.EmitEvent(const aEventName: string; const aData: ISuperObject; const aCallback:
  TSocketIOMsgJSON; const aOnError: TSocketIOError);
begin
  if aData <> nil then
...
{$IFDEF SUPEROBJECT}
procedure TSocketIOContext.SendJSON(const aJSON: ISuperObject; const aCallback: TSocketIOMsgJSON; const aOnError: TSocketIOError);
begin
  if not Assigned(aCallback) then
...
begin
  if (Value <> FContext) and (Value = nil) and (FContext is TIdServerWSContext) then
...
begin
  ...
  if (FQueue.Count > 0) or (FEvent.WaitFor(aTimeout_ms) = wrSignaled) then
  begin
    Lock;
    try
      FEvent.ResetEvent;
...
{$IFDEF SUPEROBJECT}
procedure TIdSocketIOHandling.Emit(const aEventName: string; const aData: ISuperObject; const aCallback:
  TSocketIOMsgJSON; const aOnError: TSocketIOError);
var
  context: ISocketIOContext;
  jsonarray: string;
...
function TIdSocketIOHandling.EmitSync(const aEventName: string; const aData: ISuperObject; aMaxwait_ms: Cardinal =
  INFINITE): ISuperobject;
var
...
procedure TIdSocketIOHandling.Emit(const aEventName: string; const aData: string; const aCallback: TSocketIOMsgJSON;
