unit amdws.IdServerWebsocketHandling;
...
uses
  Types, SysUtils, Classes, StrUtils, DateUtils
  // indy:
  ...
  ,amdws.IdServerBaseHandling, amdws.IdServerSocketIOHandling, amdws.IdServerWebsocketContext
;
...
type
  ...
  protected
    ...
    class procedure HandleWSMessage(AContext: TIdServerWSContext; var aType: TWSDataType; aRequestStrm, aResponseStrm:
      TMemoryStream; aSocketIOHandler: TIdServerSocketIOHandling_Ext); virtual;
  public
    class function ProcessServerCommandGet(AThread: TIdServerWSContext; ARequestInfo: TIdHTTPRequestInfo; AResponseInfo:
      TIdHTTPResponseInfo): Boolean;
...
begin
  if not (TThread.Currentthread is TIdThreadWithTask) then
    Exit(nil);
  thread := TThread.Currentthread as TIdThreadWithTask;
  if not (thread.Task is TIdServerWSContext) then
    Exit(nil);
...
begin
  ...
  try
    ...
    begin
      if context.IOHandler.HasData or (AThread.Connection.IOHandler.InputBuffer.Size > 0) or AThread.Connection.IOHandler.Readable
        (1 * 1000) then     //wait 5s, else ping the client(!)
      begin
        tstart := Now;
...
class procedure TIdServerWebsocketHandling.HandleWSMessage(AContext: TIdServerWSContext; var aType: TWSDataType;
  aRequestStrm, aResponseStrm: TMemoryStream; aSocketIOHandler: TIdServerSocketIOHandling_Ext);
begin
...
class function TIdServerWebsocketHandling.ProcessServerCommandGet(AThread: TIdServerWSContext; ARequestInfo:
...
begin
  ...
  begin
    ...
    begin
      ...
      AResponseInfo.ContentText := squid + ':15:10:websocket,xhr-polling';
    ...
    begin
      ...
      if ARequestInfo.CommandType = hcGET then
        (AThread.SocketIO as TIdServerSocketIOHandling_Ext).ProcessSocketIO_XHR(squid, ARequestInfo.PostStream,
          AResponseInfo.ContentStream)
      else if ARequestInfo.CommandType = hcPOST then
        (AThread.SocketIO as TIdServerSocketIOHandling_Ext).ProcessSocketIO_XHR(squid, ARequestInfo.PostStream, nil);   //no response expected with POST!
      Result := True;  //handled
    end
    else
  ...
  begin
    ...
    *)
    ...
    if ARequestInfo.UnparsedParams <> '' then
      context.ResourceName := context.ResourceName + '?' + ARequestInfo.UnparsedParams;
    //seperate parts
