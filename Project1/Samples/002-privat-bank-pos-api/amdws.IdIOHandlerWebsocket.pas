unit amdws.IdIOHandlerWebsocket;
...
uses
  ...
  // indy:
  ,IdGlobal, IdException, IdBuffer, IdIOHandlerStack
  {$IFDEF WEBSOCKETSSL}
  ,IdSSLOpenSSL
  {$ENDIF}
;
...
type
  ...
  {+}
  //  NEW: IIdTextEncoding declared in "IdGlobal.pas"
  //  OLD: TIdTextEncoding declared in "IdGlobal.pas"
  {$if not declared(IIdTextEncoding) and declared(TIdTextEncoding)}
    IIdTextEncoding = IdGlobal.TIdTextEncoding;
  {$ifend}
  {+.}
  ...
  public
    function WriteData(aData: TIdBytes; aType: TWSDataCode; aFIN: boolean = true; aRSV1: boolean = false; aRSV2: boolean
      = false; aRSV3: boolean = false): integer;
  ...
  private
    class var
      FInstance: TIdWebsocketWriteThread;
  public
...
procedure dbg_ws(const S: string);

var
  G_DEBUG_WS: Boolean = False;
...
uses
  Math, Windows
  // indy:
  ,IdStream, IdStack, IdWinsock2, IdExceptionCore, IdResourceStrings, IdResourceStringsCore
;

procedure dbg_ws(const S: string);
begin
  {$IFDEF MSWINDOWS}
  OutputDebugString(PChar('ws:> '+S));
  {$ELSE}
  //writeLn('ws:> ', S);
  {$ENDIF}
end;
...
begin
  try
    //valid connection?
    bConnected := Opened and SourceIsAvailable and not ClosedGracefully;

    //no socket error? connection closed by software abort, connection reset by peer, etc
    iOptLen := SIZE_INTEGER;
    bConnected := bConnected and (IdWinsock2.getsockopt(Self.Binding.Handle, SOL_SOCKET, SO_ERROR, PAnsiChar(@iOptVal),
      iOptLen) = 0) and (iOptVal = 0);
    ...
    begin
      ...
      begin
        ...
        while ReadFromSource(False{no disconnect error}, 1 * 1000, False) > 0 do
          ; //response within 1s?
      end;
    end;
  except
...
begin
  ...
  if FLock <> nil then
   FLock.Enter;
  if FSelectLock <> nil then
    FSelectLock.Enter;

  FreeAndNil(FLock);
  FreeAndNil(FSelectLock);

  FreeAndNil(FWSInputBuffer);
...
function TIdIOHandlerWebsocket.InternalReadDataFromSource(var VBuffer: TIdBytes; ARaiseExceptionOnTimeout: Boolean): Integer;
begin
  SetLength(VBuffer, 0);
...
procedure TIdIOHandlerWebsocket.WriteLn(const AOut: string; AEncoding: IIdTextEncoding);
begin
...
procedure TIdIOHandlerWebsocket.WriteLnRFC(const AOut: string; AEncoding: IIdTextEncoding);
begin
  if UseSingleWriteThread and IsWebsocket and (GetCurrentThreadID <> TIdWebsocketWriteThread.Instance.ThreadID) then
  begin
    InterlockedIncrement(FPendingWriteCount);
...
procedure TIdIOHandlerWebsocket.Write(const AOut: string; AEncoding: IIdTextEncoding);
begin
  if UseSingleWriteThread and IsWebsocket and (GetCurrentThreadID <> TIdWebsocketWriteThread.Instance.ThreadID) then
  begin
    InterlockedIncrement(FPendingWriteCount);
...
procedure TIdIOHandlerWebsocket.Write(AValue: TStrings; AWriteLinesCount: Boolean; AEncoding: IIdTextEncoding);
begin
  if UseSingleWriteThread and IsWebsocket and (GetCurrentThreadID <> TIdWebsocketWriteThread.Instance.ThreadID) then
  begin
    InterlockedIncrement(FPendingWriteCount);
...
procedure TIdIOHandlerWebsocket.Write(AStream: TStream; aType: TWSDataType);
begin
  if UseSingleWriteThread and IsWebsocket and (GetCurrentThreadID <> TIdWebsocketWriteThread.Instance.ThreadID) then
  begin
    InterlockedIncrement(FPendingWriteCount);
...
begin
  Result := -1;
  if UseSingleWriteThread and IsWebsocket
    and (GetCurrentThreadID <> TIdWebsocketWriteThread.Instance.ThreadID) then
  begin
  ...
  try
    ...
    begin
      if G_DEBUG_WS then
      begin
        dbg_ws(Format('Send (non ws, TID:%d, P:%d): %s', [GetCurrentThreadID,
          Self.Binding.PeerPort, BytesToStringRaw(ABuffer)]));
      end;
    ...
    begin
      ...
      if G_DEBUG_WS then
      begin
        dbg_ws(Format('Send (ws, TID:%d, P:%d): %s', [GetCurrentThreadID,
          Self.Binding.PeerPort, BytesToStringRaw(data)]));
      end;
      try
        DataCode := fPayloadInfo.aDataCode;
        fin := fPayloadInfo.DecLength(ALength);
        Result := WriteData(data, DataCode, fin, webBit1 in ClientExtensionBits, webBit2 in ClientExtensionBits, webBit3
...
procedure TIdIOHandlerWebsocket.ReadBytes(var VBuffer: TIdBytes; AByteCount: Integer; AAppend: Boolean);
begin
  inherited;
  if G_DEBUG_WS then
  begin
    if IsWebsocket then
    begin
      dbg_ws(Format('%d Bytes read(TID:%d): %s', [AByteCount,
        GetCurrentThreadID, BytesToStringRaw(VBuffer, AByteCount)]));
      dbg_ws(Format('Buffer (HeadIndex:%d): %s', [
...
function TIdIOHandlerWebsocket.ReadDataFromSource(var VBuffer: TIdBytes): Integer;
var
  wscode: TWSDataCode;
begin
  ...
  try
    ...
    begin
      ...
      if G_DEBUG_WS then
      begin
        dbg_ws(Format('Received (non ws, TID:%d, P:%d): %s', [GetCurrentThreadID,
          Self.Binding.PeerPort, BytesToStringRaw(VBuffer, Result)]));
      end;
    end
    else
    begin
      try
        ...
        if G_DEBUG_WS then
        begin
          dbg_ws(Format('Received (ws, TID:%d, P:%d): %s', [GetCurrentThreadID,
            Self.Binding.PeerPort, BytesToStringRaw(VBuffer)]));
        end;
...
begin
  ...
  repeat
    ...
    if (iReadCount > 0) or (lDataCode <> wdcNone) then
    begin
      ...
      case lDataCode of
        ...
        wdcClose:
          begin
            ...
            begin
              FCloseCode := (iaReadBuffer[0] shl 8) + iaReadBuffer[1];
...
begin
  Result := (FLock <> nil) and FLock.TryEnter;
end;
...
begin
  Result := IIdTextEncoding.UTF8;
end;
{$ifend}

function TIdIOHandlerWebsocket.ReadFrame(out aFIN, aRSV1, aRSV2, aRSV3: boolean; out aDataCode: TWSDataCode; out aData:
  TIdBytes): Integer;
var
  iInputPos: NativeInt;
  ...
  begin
    ...
    begin
      ...
      {if G_DEBUG_WS then
      begin
        dbg_ws(Format('Received (TID:%d, P:%d): %s', [GetCurrentThreadID,
          Self.Binding.PeerPort, BytesToStringRaw(temp)]));
        dbg_ws('Received: ' + BytesToStringRaw(temp));
      end;//}
    end;
  end;
...
begin
  ...
  begin
    ...
    begin
      iByte := _GetByte;
      iDataLength := iDataLength + (Int64(iByte) shl (8 * i)); //shift bits to left to recreate 64bit integer
    end;
  ...
  begin
    ...
    begin
      aData[iPos] := aData[iPos] xor rMask.MaskAsBytes[iPos mod 4]; //apply mask
      inc(iPos);
    end;
  end;
  ...
  if G_DEBUG_WS then
  begin
    dbg_ws(Format('Received (TID:%d, P:%d, Count=%d): %s',
      [GetCurrentThreadID, Self.Binding.PeerPort,
       Result, BytesToStringRaw(aData, Result)]));
  end;
end;
  ...
  try
    ...
    repeat
      ...
      begin
        ...
        if G_DEBUG_WS then
        begin
          dbg_ws(Format('WriteError ThrID:%d, L:%d, R:%d',
            [GetCurrentThreadID, Length(bData) - ioffset, Result]));
        end;
        break;
      end;
      Inc(ioffset, Result);
    ...
    {if G_DEBUG_WS then
    begin
      dbg_ws(Format('Written (TID:%d, P:%d): %s', [GetCurrentThreadID,
        Self.Binding.PeerPort, BytesToStringRaw(bData)]));
    end;//}
  finally
    Unlock;
    strmData.Free;
...
begin
  ...
  begin
    ...
    try
      ...
      // FInstance := nil; // Already done in "FreeAndNil"
    end;
  end;
end;
